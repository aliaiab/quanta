///Convert xkb key symbol to a quanta keyboard key
pub fn xkbKeyToQuantaKey(keysym: KeySym) ?input.KeyboardKey {
...
///"Hand rolled" key symbol enum
///This is ported from the translated c xkb headers, and has duplicate enumeration values removed
//Most of the removed duplicates are from non-english language keys, which is a shame but to support those explicitly it will be a rewrite anyway
//The rest of the duplicates were ancient sun microsystem values that are just duplicates of standard keys (f keys, pageup ect..)
//We don't even explicitly support 10% of the values anyway, so this enum will probably need more work.
//It would also be nice to change the naming conventions here, but it's a lot of work to change very little meaningfully
pub const KeySym = enum(u32) {
    ...
    ///Maximum value the backing integer can take on
    pub const maximum_integer_value = 0xffffffff - 1;
};
